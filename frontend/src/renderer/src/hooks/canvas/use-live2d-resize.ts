...
export const useLive2DResize = (
  ...
  const { updateModelScale, updateSecondModelScale } = useLive2DConfig();
    ...
    if (hasSignificantChange) {
      ...
      scaleUpdateTimeout.current = setTimeout(() => {
        if (modelIndex === 0) {
          ...
          updateSecondModelScale(smoothScale);
        }
        lastScaleRef.current = smoothScale;
      }, 500);
    }
  }, [modelRef, modelInfo?.scrollToResize, updateModelScale, updateSecondModelScale, modelIndex]);
