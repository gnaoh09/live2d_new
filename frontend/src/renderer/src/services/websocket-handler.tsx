...
function WebSocketHandler({ children }: { children: React.ReactNode }) {
  ...
  const { setModelInfo, setSecondModelInfo } = useLive2DConfig();
  ...
  const [pendingSecondModelInfo, setPendingSecondModelInfo] = useState<ModelInfo | undefined>(undefined);
  ...
  useEffect(() => {
    if (pendingSecondModelInfo) {
      setSecondModelInfo(pendingSecondModelInfo);
      setPendingSecondModelInfo(undefined);
    }
  }, [pendingSecondModelInfo, setSecondModelInfo, confUid]);
  ...
  const handleWebSocketMessage = useCallback((message: MessageEvent) => {
    ...
    switch (message.type) {
      ...
      case 'config-switched':
        ...
        // Clear both model infos when switching configs
  ...
  }, [aiState, addAudioTask, appendHumanMessage, baseUrl, bgUrlContext, setAiState, setConfName, setConfUid, setConfigFiles, setCurrentHistoryUid, setHistoryList, setMessages, setModelInfo, setSecondModelInfo, setSubtitleText, startMic, stopMic, setSelfUid, setGroupMembers, setIsOwner, backendSynthComplete, setBackendSynthComplete, clearResponse]);
